...
| 2. Dataset, preprocessing, and split evidence | `docs/DATA_STATEMENT.md`; `docs/EXPERIMENT_PIPELINE.md` Section 1; `results_public/tables/dataset_split_statistics.csv` | Covered. Raw data are excluded. |
| 3. Training, backbone comparison, and multi-seed robustness | `docs/EXPERIMENT_PIPELINE.md` Sections 2 and 8; `docs/RESEARCH_REPORT.md` Sections 4 and 9; `results_public/tables/model_performance_and_geometry.csv` | Covered. |
| 4. Embedding geometry: 2D and 3D projections | `docs/RESEARCH_REPORT.md` Section 5; `results_public/figures/01_embedding_pca/` Figures 001-026 | Covered with public-safe projection figures. |
| 5. Best-model uncertainty, ambiguity, and review curves | `docs/RESEARCH_REPORT.md` Sections 6 and 10; `results_public/tables/uncertainty_error_detection.csv`; `results_public/figures/02_uncertainty_calibration/` Figures 027-036 | Covered. |
| 6. ECG regularity features and interpretability | `docs/RESEARCH_REPORT.md` Section 8; `src/regularity_analysis.py`; `src/regularity_feature_ablation.py`; `results_public/figures/03_regularity_interpretability/` Figures 037-042 | Covered. |
| 7. Full-spectrum shift and OOD-style perturbation benchmark | `docs/RESEARCH_REPORT.md` Section 7; `src/evaluate_ood.py`; `src/evaluate_corruption_severity.py`; `results_public/figures/04_ood_corruption/` Figures 043-053 | Covered. |
| 8. Risk head, safety supervisor, and ablation evidence | `docs/RESEARCH_REPORT.md` Section 10; `src/runtime_supervisor.py`; `src/risk_head_review_analysis.py`; `results_public/figures/05_risk_supervisor_ablation/` Figures 054-066 | Covered. |
...
| Appendix A. Full raw image archive | `results_public/figures/` contains only public-safe grouped figures | Raw archive intentionally not published. |
...
| V4 PRO, RISK, auxiliary ablations, severity robustness | `docs/RESEARCH_REPORT.md` Sections 9-10; `results_public/figures/06_pro_geometry/` and `results_public/figures/07_pro_severity_robustness/` | Covered. |
...
| V5 review-routing reliability, RISK corruption robustness, evidence cards, code explanation | `docs/RESEARCH_REPORT.md` Sections 7, 10, and 11; `src/README.md`; `results_public/figures/08_risk_corruption_robustness/` and later V5/V6 figure groups | Covered, except evidence cards are summarized rather than published. |
