...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CNN+TCN validity v2 with a VT/VF specialist mixture.")
    parser.add_argument("--mat", type=Path, default=Path("RHYTHMS.mat"))
    parser.add_argument("--out", type=Path, default=Path("results/cnn_tcn_validity_v2_20260627"))
    ...
    manifest = args.out / f"cnn_tcn_validity_v2_manifest_{stamp}.csv"
    manifest.write_text("\n".join(manifest_rows) + "\n", encoding="utf-8")
